...
- **チャットコマンド**
  ...
  - **コマンド一覧**
    ...
    |書式|別名|説明|
    |---|---|---|
    |musk \<pattern>||ブラシにマスクをセットします。マスクでセットしたブロックのみをブラシで置き換えるようになります。<br>\<pettern>未入力でマスクをリセットできます。<br><br>**\<pattern>**: 配置するブロックのパターン。<br>&emsp;**blockId**: ブロックのID。(例: `minecraft:stone、air`)<br>&emsp;**main**: メインハンドのブロックを指定。<br>&emsp;**view**: 視点先のブロックを指定。(blockStatesを所持)|
